<?php
$cssCode = '';
if (get_theme_mod('otf_colors_header_skin', 'light') === 'custom' || is_customize_preview()) {
    $header_bg_color = ariColor::new_color(get_theme_mod('otf_colors_header_bg', '#fff'));
    $main_menu_color = get_theme_mod('otf_colors_mainmenu');
    $header_color = get_theme_mod('otf_colors_header_color', '#000');
    $header_dark_bg_color = $header_bg_color->get_new('lightness', $header_bg_color->lightness - 10)->to_css('rgba');
    $topbar_bg_color = ariColor::new_color(get_theme_mod('otf_colors_topbar_bg', '#ddd'));
    $topbar_bg_hover = $topbar_bg_color->get_new('lightness', $topbar_bg_color->lightness - 20)->to_css('rgba');
    $topbar_color = get_theme_mod('otf_colors_topbar_color', '#222');
    if (empty($main_menu_color)) {
        $main_menu_color = $header_color;
    }

    $color_header_mobile = ariColor::color_light_or_dark(get_theme_mod('otf_colors_header_bg', '#ffffff'));
    $cssCode
        .= <<<CSS
...
.opal-header-skin-custom .site-header .site-header-mobile .header-button,
.opal-header-skin-custom .site-header .site-header-mobile input{
    color: $color_header_mobile;
}
...
if (get_theme_mod('otf_colors_header_sticky_skin', 'light') === 'custom' || is_customize_preview()) {
    $header_sticky_bg_color = ariColor::new_color(get_theme_mod('otf_colors_header_sticky_bg', '#fff'));
    $header_sticky_color = get_theme_mod('otf_colors_header_sticky_color', '#000');
    $main_menu_sticky_color = get_theme_mod('otf_colors_mainmenu_sticky');
...
.opal-header-sticky-skin-custom #opal-header-sticky .menu-toggle{
...
if (get_theme_mod('otf_header_layout_enable_side_header', false)) {
    $side_header_width = get_theme_mod('otf_header_layout_side_header_width', 300);
