...
$cssCode = $body_code = $tertiary_code = $quaternary_code = '';
...
if ( is_array( $tertiary_font ) && $tertiary_font['family'] ) {
	$tertiary_code .= "font-family:\"{$tertiary_font['family']}\", -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif;";
	$tertiary_code .= "font-weight: {$tertiary_font['fontWeight']};";
}
$quaternary_font = get_theme_mod( 'otf_typography_general_quaternary_font' );
if ( is_array( $quaternary_font ) && $quaternary_font['family']) {
	$quaternary_code .= "font-family:\"{$quaternary_font['family']}\", -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif;";
	$quaternary_code .= "font-weight: {$quaternary_font['fontWeight']};";
}
$body_font_size = get_theme_mod( 'otf_typography_general_body_font_size', 15 );
if ( $body_font_size && $body_font_size != 15 ) {
	$body_code .= "font-size: {$body_font_size}px;";
}

$body_letter_spacing = get_theme_mod( 'otf_typography_general_body_letter_spacing', 0 );
if ( $body_letter_spacing && $body_letter_spacing != 0 ) {
	$body_code .= "letter-spacing: {$body_letter_spacing}px;";
}

$body_line_height = get_theme_mod( 'otf_typography_general_body_line_height', 24 );
if ( $body_line_height && $body_line_height != 24 ) {
	$body_code .= "line-height: {$body_line_height}px;";
}

$body_color = get_theme_mod( 'otf_colors_general_body', '#222' );
if ( $body_color ) {
	$body_code .= "color: {$body_color}";
}

$border_color  = get_theme_mod( 'otf_colors_general_border', '#ebebeb' );
$heading_color = get_theme_mod( 'otf_colors_general_heading', '#111' );

$color_primary         = ariColor::new_color( get_theme_mod( 'otf_colors_general_primary', '#0160b4' ) );
$color_primary_hover   = $color_primary->get_new( 'lightness', $color_primary->lightness - 10 );
$color_secondary       = ariColor::new_color( get_theme_mod( 'otf_colors_general_secondary', '#00c484' ) );
...
return apply_filters( 'otf_customize_colors', $cssCode, $color_primary, $color_primary_hover, $color_secondary, $color_secondary_hover, $body_color, $heading_color, $border_color );
