...
final class CEI_Core {
    ...
    static public function init($wp_customize) {
        if (current_user_can( 'edit_theme_options' )){
            ...
            if (isset( $_REQUEST['cei-export-all'] )){
                self::_export_all( $wp_customize );
            }

            if (isset( $_REQUEST['cei-export-settings'] )){
                self::_export_settings( $wp_customize );
            }
    ...
    static public function controls_enqueue_scripts() {
        ...
        wp_localize_script( 'cei-js', 'CEIl10n', array(
            'emptyImport' => __( 'Please choose a file to import.', 'opal-theme-framework' ),
        ) );
    ...
    static public function register($wp_customize) {
        ...
        $wp_customize->add_section( 'cei-section', array(
            'title'    => __( 'Export/Import', 'opal-theme-framework' ),
            'priority' => 10000000,
        ) );
    ...
    static private function _export_all($wp_customize) {
        if (!wp_verify_nonce( $_REQUEST['cei-export-all'], 'cei-exporting' )){
            return;
        }
        ...
        $mods     = get_theme_mods();
        $data     = array(
            'template' => apply_filters( 'otf_customize_export_template_name', $template ),
        ...
        $dir    = trailingslashit(ABSPATH) . 'sample_data/';
        if (session_id() == ''){
            session_start();
        }
        if(isset($_SESSION['opal-theme'])){
            $folder = $_SESSION['opal-theme'];
        }else{
            $folder = wp_basename( get_stylesheet_directory() );
        }
        if (!is_dir( $dir )){
            mkdir( $dir, 0777 );
        }
        ...
        $fp = fopen( $dir . $folder . '/customizer.dat', 'w' );
        fwrite( $fp, serialize( $data ) );
        fclose( $fp );

        do_action( 'otf-export-all-file' );
        die;
    }

    static private function _export_settings($wp_customize) {
    //        if (!wp_verify_nonce( $_REQUEST['cei-export-settings'], 'cei-exporting' )){
    //            return;
    //        }
        ...
        do_action( 'otf-export-settings-file' );
    }
    ...
    static private function _import($wp_customize) {
        ...
        if (!file_exists( $file['file'] )){
            $cei_error = __( 'Error importing settings! Please try again.', 'opal-theme-framework' );
        ...
        if ('array' != gettype( $data )){
            $cei_error = __( 'Error importing settings! Please check that you uploaded a customizer export file.', 'opal-theme-framework' );
        ...
        //            $cei_error = __( $template . ' - ' . $data['template'] . ' - Error importing settings! The settings you uploaded are not for the current theme.', 'customizer-export-import' );
        //
        //            return;
        //        }
