    ...
    public function __construct() {
        ...
        if ('vc_get_autocomplete_suggestion' === vc_request_param('action') || 'vc_edit_form' === vc_post_param('action')) {
            ...
            if (otf_is_woocommerce_activated()) {
                add_filter('vc_autocomplete_otf_product_parallax_id_callback', array(
                    OTF_WooCommerce::getInstance(),
                    'productIdAutocompleteSuggester'
                ), 10, 1);
                add_filter('vc_autocomplete_otf_product_parallax_id_render', array(
                    OTF_WooCommerce::getInstance(),
                    'productIdAutocompleteRender'
                ), 10, 1);

                add_filter('vc_autocomplete_otf_product_category_banner_category_callback', array(
                    OTF_WooCommerce::getInstance(),
                    'productCategoryAutocompleteSuggester'
                ), 10, 1);
                add_filter('vc_autocomplete_otf_product_category_banner_category_render', array(
                    OTF_WooCommerce::getInstance(),
                    'productCategoryAutocompleteRender'
                ), 10, 1);

                add_filter('vc_autocomplete_otf_product_banner_id_callback', array(
                    OTF_WooCommerce::getInstance(),
                    'productIdAutocompleteSuggester'
                ), 10, 1);
                add_filter('vc_autocomplete_otf_product_banner_id_render', array(
                    OTF_WooCommerce::getInstance(),
                    'productIdAutocompleteSuggester'
                ), 10, 1);
            }
...
//        add_action('vc_after_init', array($this, 'add_field_to_vc_gitem_post_excerpt'), 11);
        ...
        add_filter('vc_gitem_template_attribute_post_categories', array(
            $this,
            'gitem_template_attribute_post_categories'
        ), 11, 2);
        add_filter('vc_gitem_template_attribute_post_excerpt', array(
            $this,
            'gitem_template_attribute_post_excerpt'
        ), 11, 2);
...
//        $el_class = $this->getExtraClass( $el_class );
    ...
    public function add_field_to_wp_custommenu() {
        $args = array(
            ...
            array(
                'type'       => 'dropdown',
                'heading'    => __('List Style', 'opal-theme-framework'),
                'param_name' => 'opal_list_style',
                'group'      => esc_html__('Opal Extras', 'opal-theme-framework'),
                'value'      => array(
                    esc_html__('None', 'opal-theme-framework')           => 'none',
                    esc_html__('Round', 'opal-theme-framework')          => 'list-round',
                    esc_html__('Square', 'opal-theme-framework')         => 'list-square',
                    esc_html__('Round Outline', 'opal-theme-framework')  => 'list-round-outline',
                    esc_html__('Square Outline', 'opal-theme-framework') => 'list-square-outline',
    ...
    public function custom_output($output, $obj, $attr) {
        if (!empty($attr['el_aspect_custom'])) {
            $ratio = $attr['el_aspect_custom'];
            if (is_string($ratio)) {
                preg_match_all('/\d+/', $ratio, $ratio_matches);
                if (isset($ratio_matches[0])) {
                    $ratio = array();
                    if (count($ratio_matches[0]) > 1) {
                        $ratio[] = $ratio_matches[0][0]; // width
                        $ratio[] = $ratio_matches[0][1]; // height
                    } elseif (count($ratio_matches[0]) > 0 && count($ratio_matches[0]) < 2) {
                        $ratio[] = $ratio_matches[0][0]; // width
                        $ratio[] = $ratio_matches[0][0]; // height
                    } else {
                        $ratio = false;
                    }
                }
                $ratio  = $ratio[1] / $ratio[0] * 100;
                $css    = 'style="padding-top:' . $ratio . '%"';
                $output = preg_replace('/wpb_video_wrapper"/', 'wpb_video_wrapper" ' . $css, $output, 1);
            }
        }
        ...
        if (!empty($attr['opal_custommenu_columns']) || !empty($attr['opal_list_style'])) {
            if (!empty($attr['opal_custommenu_columns'])) {
                $output = str_replace('<ul', '<ul style="columns:' . $attr['opal_custommenu_columns'] . ';"', $output);
            }
            if (!empty($attr['opal_list_style'])) {
                $output = '<div class="opal-custom-menu-' . esc_attr($attr['opal_list_style']) . '">' . $output . '</div>';
            }
    ...
    public function add_field_to_video() {
        vc_add_params('vc_video', array(
            array(
                'type'       => 'textfield',
                'heading'    => __('Video aspect ration', 'opal-theme-framework'),
                'param_name' => 'el_aspect_custom',
                'group'      => esc_html__('Opal Extras', 'opal-theme-framework'),
                'dependency' => array(
                    'element' => 'el_aspect',
                    'value'   => 'custom'
        ...
        $params = vc_get_shortcode('vc_video')['params'];
        foreach ($params as $key => $param) {
            if ($param['param_name'] === 'el_aspect') {
                $params[$key]['value']['Opal custom'] = 'custom';
                break;
            }
        }
