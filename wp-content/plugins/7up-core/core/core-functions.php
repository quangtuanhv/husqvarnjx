...
if(!function_exists('s7upf_scrape_instagram')){
    function s7upf_scrape_instagram($username, $slice = 9 , $token = '',$size_index= 0) {
        ...
        if($username) {
            ...
            $followed_by = $insta_array['entry_data']['ProfilePage'][0]['graphql']['user']['edge_followed_by']['count'];
            $count_photos = $insta_array['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['count'];
        ...
        $instagram = array_slice($instagram, 0, $slice);
        $data = array(
