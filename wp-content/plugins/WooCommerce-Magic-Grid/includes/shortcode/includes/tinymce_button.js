(function(){
	...
	tinymce.PluginManager.add('flash_sale_shortcodes_button', function( editor, url ) {
        editor.addButton( 'flash_sale_shortcodes_button', {
            title: 'Add Woo Magic Grid',
            icon: 'fa-check',
            onclick: function() {
				var width = jQuery(window).width(), H = jQuery(window).height(), W = ( 720 < width ) ? 720 : width;
				W = W - 80;
				H = H - 84;
				tb_show( 'WooCommerce Magic Grid', '#TB_inline?width=' + W + '&height=' + H + '&inlineId=flash_sale-shortcodes-form' );
            }
        });
    });
...
#flash_sale-shortcodes-form-tab_accordion .flash_sale-shortcodes-toggle-icon-list { margin: 0 0 10px 0; }\
		...
		<div class="flash_sale-shortcodes-form-types">\
			<ul>\
				<li class="flash_sale-shortcodes-form-type-button active" type="ad-search-grid"><a href="#"></a></li>\
			</ul>\
		</div><!-- end types -->\
		<div class="flash_sale-shortcodes-form-tabs">\
<!-- product_rule -->\
			<div class="flash_sale-shortcodes-form-tab" id="flash_sale-shortcodes-form-tab_ad-search-grid" style="display:block">\
				<h2>Add Magic Grid Shortcode</h2>\
				<div class="flash_sale-shortcodes-form-fields">\
				<table cellpadding="0" cellspacing="0">\
					<tr>\
						<th><label>Choose Shortcode</label></th>\
						<td>\
							<select fieldname="id" id="select_rule">\
		...
		jQuery.ajax ({
			type: "POST",
			url: ajaxurl,
			data:   "action=pw_woo_ad_search_fetch_shortcode",
		...
		var flash_sale_shortcode_type = "ad-search-grid";
		...
		form.find('#flash_sale_shortcodes-submit').click(function(){
			...
			//	flash_sale_shortcode_code = flash_sale_shortcode_code + '[/flash_sale_' + flash_sale_shortcode_type + ']';
				...
				/* Basic shortcodes */
				flash_sale_shortcode_code = flash_sale_shortcode_code + '[pw-woo-' + flash_sale_shortcode_type + ' ';
