...
class Yog_WC_Shortcode_Products {
	...
	protected function product_loop() {
		...
		if ( $products && $products->ids ) {
            ...
            $yog_counter = 1; $yog_columns = $columns; $yog_close = false;
            if ( wc_get_loop_prop( 'total' ) ) {
				foreach ( $products->ids as $product_id ) {
    				...
    				if( $this->attributes['style'] == 'one' ){
                        ...
                        wc_get_template_part( 'woo-woocommerce/content-product-slider', 'v1' );
                        
                    }elseif( $this->attributes['style'] == 'two' ){
                        
                        wc_get_template( 'woo-woocommerce/content-product-list.php' , array( 'classes' => $classes ) );
                        ...
                        wc_get_template( 'woo-woocommerce/content-product-grid.php' , array( 'classes' => $classes ) );
                        
                        //Row Wrapper Class
                        if( $yog_counter == $columns ):
            ...
            if( $this->attributes['paginate'] ){
                yog_wp_paginate( array( 'query' => $products->query,  'before' => '<div class="toolbar"><div class="pager"><div class="pages">', 'after' => '</div></div></div>', 'class' => 'pagination', 'title' => false, 'nextpage' => '<i class="fa fa-angle-right"></i>', 'previouspage' => '<i class="fa fa-angle-left"></i>' ) ); //Paginations
            }
	...
	function yog_woocommerce_product_loop_start( $atts = array() ) {
        ...
        if( $atts['style'] == 'one' ){
            ?>
            <div class="best-pro slider-items-products" <?php yog_helper()->attr( false, array( 'data-animation' => $yog_animation, 'data-animation-daley' => $yog_delay ) ) ?>>
                <?php 
                    //Heading
        ...
        }elseif( $atts['style'] == 'three' ){
            ?>
            <div class="category-products" <?php yog_helper()->attr( false, array( 'data-animation' => $yog_animation, 'data-animation-daley' => $yog_delay ) ) ?>>
