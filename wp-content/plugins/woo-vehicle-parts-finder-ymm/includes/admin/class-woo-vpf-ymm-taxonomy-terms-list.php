 ...
 *
 * @class WOO_VPF_YMM_Taxonomy_Terms_List
 */
...
/**
 * WOO_VPF_YMM_Taxonomy_Terms_List Class
 */
class WOO_VPF_YMM_Taxonomy_Terms_List {
	...
	public function init() {
		...
		// Assign Labels
		$this->label_year = WOO_VPF_YMM_Functions::get_year_label();
		$this->label_make = WOO_VPF_YMM_Functions::get_make_label();
		$this->label_model = WOO_VPF_YMM_Functions::get_model_label();
		$this->label_engine = WOO_VPF_YMM_Functions::get_engine_label();
		...
		if( $pagenow != 'term.php' ) {
			...
			if( isset( $_REQUEST['parent'] ) && $_REQUEST['parent'] > 0 ) {
				$this->parent_terms = WOO_VPF_YMM_Functions::get_parent_terms_hierarchy( $_REQUEST['parent'] );
			}
		...
		if( $pagenow == 'edit-tags.php' || $pagenow == 'term.php' ) {
			...
			if( ! ( WOO_VPF_YMM_Functions::is_wpml_activated() && isset( $_REQUEST['trid'] ) ) ) {
	...
	public function terms_list_columns( $columns ) {
		...
		$columns['posts']	= __( 'Count', WOO_VPF_YMM_TEXT_DOMAIN );
	...
	public function terms_list_row_actions( $actions, $term ) {
		...
		if( isset( $this->term_levels[ $this->current_term_level['level'] ] ) ) {
			...
			$actions['view_child_terms'] = sprintf( '<a href="%s" aria-label="%s">%s</a>', $view_child_terms_link, sprintf( __( 'View %s', WOO_VPF_YMM_TEXT_DOMAIN ), $next_level['label'] ), sprintf( __( 'View %s', WOO_VPF_YMM_TEXT_DOMAIN ), $next_level['label'] ) );
		}
	...
	public function terms_delete_form() {
		?><form class="woo-vpf-ymm-terms-delete-form" method="post">
			...
			<input type="submit" value="<?php _e( 'Delete All Terms', WOO_VPF_YMM_TEXT_DOMAIN ); ?>" class="button button-primary" />
		</form><?php
	}
	...
	public function terms_search_form() {
		?><div class="form-wrap woo-vpf-ymm-form-container woo-vpf-ymm-search-form-container">
			<h2><?php _e( 'Search Terms', WOO_VPF_YMM_TEXT_DOMAIN ); ?></h2>
			...
			<form method="get">
				...
				<?php
					...
					if( isset( $_REQUEST['parent'] ) && $_REQUEST['parent'] > 0 ) {
						$parent_terms = WOO_VPF_YMM_Functions::get_parent_terms_hierarchy( $_REQUEST['parent'] );
					}
				
					if( $this->current_term_level['level'] > 1 ) {
						?><div class="form-field">
							<select class="woo_vpf_ymm_year">
								<option value=""><?php printf( __( '-- Select %s --', WOO_VPF_YMM_TEXT_DOMAIN ), $this->label_year ); ?></option>
			
								<?php WOO_VPF_YMM_Functions::get_terms_list( 0, isset( $parent_terms['year']['id'] ) ? $parent_terms['year']['id'] : '' ); ?>
							</select>
						</div><?php
					}
					...
					if( $this->current_term_level['level'] > 2 ) {
						?><div class="form-field">
							<select class="woo_vpf_ymm_make">
								<option value=""><?php printf( __( '-- Select %s --', WOO_VPF_YMM_TEXT_DOMAIN ), $this->label_make ); ?></option>
			
								<?php WOO_VPF_YMM_Functions::get_terms_list( isset( $parent_terms['year']['id'] ) ? $parent_terms['year']['id'] : '', isset( $parent_terms['make']['id'] ) ? $parent_terms['make']['id'] : '' ); ?>
							</select>
						</div><?php
					}
					...
					if( $this->current_term_level['level'] > 3 ) {
						?><div class="form-field">
							<select class="woo_vpf_ymm_model">
								<option value=""><?php printf( __( '-- Select %s --', WOO_VPF_YMM_TEXT_DOMAIN ), $this->label_model ); ?></option>
		
								<?php WOO_VPF_YMM_Functions::get_terms_list( isset( $parent_terms['make']['id'] ) ? $parent_terms['make']['id'] : '', isset( $parent_terms['model']['id'] ) ? $parent_terms['model']['id'] : '' ); ?>
							</select>
						</div><?php
					}
				?>
				...
				<div class="form-field">
					<input type="text" name="s" placeholder="<?php _e( 'Keyword', WOO_VPF_YMM_TEXT_DOMAIN ); ?>" value="<?php echo ( isset( $_REQUEST['s'] ) ? $_REQUEST['s'] : '' ); ?>" />
				</div>
				
				<p class="submit">
					<input type="submit" value="<?php _e( 'Submit', WOO_VPF_YMM_TEXT_DOMAIN ); ?>" class="button button-primary" />
				</p>
			</form>
		</div><?php
	}
...
new WOO_VPF_YMM_Taxonomy_Terms_List();
