 ...
 *
 * @class WOO_VPF_YMM_Taxonomy_Metabox_Template
 */
...
/**
 * WOO_VPF_YMM_Taxonomy_Metabox_Template Class
 */
class WOO_VPF_YMM_Taxonomy_Metabox_Template {
	...
	public function add_taxonomy_meta_box() {
		...
		add_meta_box( 'product_custom_ymmdiv', sprintf( '%s  <a href="javascript:" class="woo-vpf-ymm-add-term-row">' . __( 'Add', WOO_VPF_YMM_TEXT_DOMAIN ) . '</a>', __( 'VPF ( YMM ) Terms', WOO_VPF_YMM_TEXT_DOMAIN ) ), array( $this, 'taxonomy_meta_box' ), 'product', 'normal' );
	}
	...
	public function taxonomy_meta_box( $post ) {
		...
		$label_year = WOO_VPF_YMM_Functions::get_year_label();
		$label_make = WOO_VPF_YMM_Functions::get_make_label();
		$label_model = WOO_VPF_YMM_Functions::get_model_label();
		$label_engine = WOO_VPF_YMM_Functions::get_engine_label();
		...
		?><div class="woo-vpf-ymm-form-container">
			...
			<table class="form-table">
				<tbody>
					<tr>
						<td>
							<label for="vpf_ymm_universal"><?php _e( 'Universal Product?', WOO_VPF_YMM_TEXT_DOMAIN ); ?></label>
							<input type="checkbox" name="vpf_ymm_universal" class="vpf_ymm_universal" id="vpf_ymm_universal" value="1" <?php if( $_universal ) { echo 'checked'; } ?> />
						</td>
						
						<td>
							<a href="javascript:" class="button button-primary woo-vpf-ymm-remove-term-rows"><?php _e( 'Delete All Terms', WOO_VPF_YMM_TEXT_DOMAIN ); ?></a>
						</td>
					</tr>
				</tbody>
			</table>
				...
				<table class="form-table woo-vpf-ymm-form-table">
					<thead>
						<tr>
							...
							<th scope="row" class="woo-vpf-ymm-text-center"><?php _e( 'Actions', WOO_VPF_YMM_TEXT_DOMAIN ); ?></th>
						</tr>
					</thead>
	...
	public function taxonomy_meta_box_row( $post_term=array(), $classes=array() ) {
		?><tr class="woo-vpf-ymm-terms-row <?php echo ( empty( $post_term ) ? 'woo-vpf-ymm-terms-row-add' : 'woo-vpf-ymm-terms-row-edit' ); ?> <?php if( ! empty( $classes ) ) { echo implode( ' ', $classes ); } ?>">
			<td>
				<select name="woo_vpf_ymm[]" class="woo_vpf_ymm_year">
					<option value=""><?php _e( '-- Select --', WOO_VPF_YMM_TEXT_DOMAIN ); ?></option>
					
					<?php WOO_VPF_YMM_Functions::get_terms_list( 0, ( ! empty( $post_term ) && isset( $post_term['year']['id'] ) ) ? $post_term['year']['id'] : '' ); ?>
				</select>
			</td>
				...
				<select name="woo_vpf_ymm[]" class="woo_vpf_ymm_make">
					...
					<?php
						if( ! empty( $post_term ) && isset( $post_term['year']['id'] ) ) {
							WOO_VPF_YMM_Functions::get_terms_list( $post_term['year']['id'], isset( $post_term['make']['id'] ) ? $post_term['make']['id'] : '' );
						}
					?>
				</select>
			</td>
				...
				<select name="woo_vpf_ymm[]" class="woo_vpf_ymm_model">
					...
					<?php
						if( ! empty( $post_term ) && isset( $post_term['make']['id'] ) ) {
							WOO_VPF_YMM_Functions::get_terms_list( $post_term['make']['id'], isset( $post_term['model']['id'] ) ? $post_term['model']['id'] : '' );
						}
					?>
				</select>
			</td>
			...
			<td>
				<select name="woo_vpf_ymm[]" class="woo_vpf_ymm_engine" <?php if( empty( $post_term ) ) { echo 'multiple'; } ?> >
					<option value=""><?php _e( '-- Select --', WOO_VPF_YMM_TEXT_DOMAIN ); ?></option>
					
					<?php
						if( ! empty( $post_term ) && isset( $post_term['model']['id'] ) ) {
							WOO_VPF_YMM_Functions::get_terms_list( $post_term['model']['id'], isset( $post_term['engine']['id'] ) ? $post_term['engine']['id'] : '' );
						}
					?>
				</select>
			</td>
			...
			<td class="woo-vpf-ymm-term-actions woo-vpf-ymm-text-center">
				<a href="javascript:" class="woo-vpf-ymm-remove-term-row"><?php _e( 'Delete', WOO_VPF_YMM_TEXT_DOMAIN ); ?></a>
			</td>
		</tr><?php
	}
	...
	public function save_taxonomy_meta_box( $post_id ) {
		...
		if( ! isset( $_POST['product_ymm_taxonomy_noncename'] ) || ! wp_verify_nonce( $_POST['product_ymm_taxonomy_noncename'], 'taxonomy_product_ymm' ) ) return;
...
new WOO_VPF_YMM_Taxonomy_Metabox_Template();
