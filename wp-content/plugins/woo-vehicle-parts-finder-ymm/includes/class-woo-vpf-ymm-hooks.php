...
class WOO_VPF_YMM_Hooks {
	...
	public function __construct() {
		...
		// Filter query hook to include WOO_VPF_YMM search criteria
	...
	/**
	 * Include WOO_VPF_YMM query params in the query
	 */
	...
	/**
	 * Override search page title with WOO_VPF_YMM title ( Breadcrumbs / Page Meta Title )
	 */
	function search_page_html_esc_title( $title ) {
		if( WOO_VPF_YMM_Functions::is_search() ) {
			$title = WOO_VPF_YMM_Hooks::search_page_title( $title );
			$title = wp_strip_all_tags( $title );
			
			$title_search_results_label = WOO_VPF_YMM_Functions::get_search_results_label();
	...
	/**
	 * Override search page title with WOO_VPF_YMM title
	 */
		...
		if( WOO_VPF_YMM_Functions::is_search() ) {
			...
			if( isset( $_REQUEST['year_id'] ) ) {
				...
				if( $year > 0 ) {
					...
					if( !empty( $term_year ) ) {
						$title_year = sprintf( __( '<span class="search_col search_col_year">%s: <span class="search_col_val">%s</span></span>,', WOO_VPF_YMM_TEXT_DOMAIN ), WOO_VPF_YMM_Functions::get_year_label(), $term_year->name );
					}
				}
			}
			...
			if( isset( $_REQUEST['make'] ) ) {
				...
				if( $make > 0 ) {
					$term_make = get_term( $make, 'product_ymm' );
					if( !empty( $term_make ) ) {
						$title_make = sprintf( __( '<span class="search_col search_col_make">%s: <span class="search_col_val">%s</span></span>,', WOO_VPF_YMM_TEXT_DOMAIN ), WOO_VPF_YMM_Functions::get_make_label(), $term_make->name );
					}
				}
			}
			...
			if( isset( $_REQUEST['model'] ) ) {
				...
				if( $model > 0 ) {
					$term_model = get_term( $model, 'product_ymm' );
					if( !empty( $term_model ) ) {
						$title_model = sprintf( __( '<span class="search_col search_col_model">%s: <span class="search_col_val">%s</span></span>,', WOO_VPF_YMM_TEXT_DOMAIN ), WOO_VPF_YMM_Functions::get_model_label(), $term_model->name );
					}
				}
			}
			...
			if( isset( $_REQUEST['engine'] ) ) {
				...
				if( $engine > 0 ) {
					...
					if( !empty( $term_engine ) ) {
						$title_engine = sprintf( __( '<span class="search_col search_col_engine">%s: <span class="search_col_val">%s</span></span>,', WOO_VPF_YMM_TEXT_DOMAIN ), WOO_VPF_YMM_Functions::get_engine_label(), $term_engine->name );
					}
				}
			}
			...
			if( isset( $_REQUEST['category'] ) ) {
				...
				if( $category > 0 ) {
					...
					if( !empty( $term_category ) ) {
						$title_category = sprintf( __( '<span class="search_col search_col_category">%s: <span class="search_col_val">%s</span></span>,', WOO_VPF_YMM_TEXT_DOMAIN ), WOO_VPF_YMM_Functions::get_category_label(), $term_category->name );
					}
				}
			}
			...
			if( isset( $_REQUEST['s'] ) ) {
				...
				if( $s != '' ) {
					$title_s = sprintf( __( '<span class="search_col search_col_keyword">%s: <span class="search_col_val">%s</span></span>,', WOO_VPF_YMM_TEXT_DOMAIN ), WOO_VPF_YMM_Functions::get_keyword_label(), $s );
				}
			}
			...
			$title = sprintf( __( '%s %s %s %s %s %s %s', WOO_VPF_YMM_TEXT_DOMAIN ), $title_search_results_label, $title_year, $title_make, $title_model, $title_engine, $title_category, $title_s );
		...
		if( $woo_vpf_ymm_activate_tab == 'yes' ) {
			...
			if( ! empty( $terms ) ) {
				...
				if( $woo_vpf_ymm_tab_title == '' ) {
					$woo_vpf_ymm_tab_title = __( 'Fit for the Following Vehicles', WOO_VPF_YMM_TEXT_DOMAIN );
				}
	...
	function product_tab_content() {
		...
		list( $post_terms, $years, $makes, $models, $engines ) = WOO_VPF_YMM_Functions::wp_get_post_terms_hierarchy( $post->ID );
		...
		$year_label = WOO_VPF_YMM_Functions::get_year_label();
		$make_label = WOO_VPF_YMM_Functions::get_make_label();
		$model_label = WOO_VPF_YMM_Functions::get_model_label();
		$engine_label = WOO_VPF_YMM_Functions::get_engine_label();
							...
							?><tbody><?php
								...
								if( ! empty( $terms ) ) {
									...
									if( $woo_vpf_ymm_activate_tab_year_ranges == 'yes' ) {
										$terms = WOO_VPF_YMM_Functions::arrange_terms_by_year_range( $terms );
									}
									
									$terms = WOO_VPF_YMM_Functions::sort_terms( $terms );
