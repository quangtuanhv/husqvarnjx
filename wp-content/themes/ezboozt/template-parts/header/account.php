...
$icon = !empty($atts['icon']) ? $atts['icon'] : 'icon icon-1222';
$icon_style = !empty($atts['i_style']) ? $atts['i_style'] : 'font-size:24px;line-height:36px;';
...
<div class="site-header-account d-inline-block  <?php echo esc_attr($classes); ?>">
    <?php if (!is_user_logged_in()) {
        echo '<a href="' . esc_html($account_link) . '" title="' . esc_html__('Login', 'ezboozt') . '">
                <span class="' . esc_attr($icon) . '" style="' . esc_attr($icon_style) . '"></span>
                <span class="account-label">
                    <span>' . esc_html__('Login', 'ezboozt') . '</span>
                    <span>' . esc_html__('Register', 'ezboozt') . '</span>
                </span>
              </a>';
    } else {
