<?php if( yog_helper()->get_option( 'header-restaurant-cart' ) && class_exists('Woocommerce') ){  ?>

<div class="dropdown d-inline-block mr-2">

   <?php do_action( 'woocommerce_before_mini_cart' ); ?>
   <a class="nav-link dropdown-toggle cart-dropdown after_none p-0 pr-3 position-relative" href="#" id="cartdetails" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
     <i class="fas fa-shopping-bag"></i> 
     <div class="cart-dropdown_count bg-primary"><?php echo WC()->cart->get_cart_contents_count(); ?></div>
   </a> 
         
   <div class="dropdown-menu" aria-labelledby="cartdetails" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 28px, 0px); top: 0px; left: 0px; will-change: transform;">
        <ul>
        		...
        		<?php
        			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
        				...
        				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
        					...
        					$thumbnail         = apply_filters( 'woocommerce_cart_item_thumbnail', $_product->get_image(array( 35,35 )), $cart_item, $cart_item_key );
