...
function yog_get_products_per_page(){
    ...
    if(isset($_POST['yog-woocommerce-products-per-page'])){
        ...
        return yog_helper()->get_option( 'shop-limit' );
...
function woocommerceframework_pagination() {
    if( yog_helper()->get_option( 'shop-pagination', 'raw', false, 'options' ) ){
        yog_wp_paginate( array( 'before' => '<div class="clearfix filters-container m-b-20"><div class="text-right"><div class="pagination-container">', 'after' => '</div></div></div>', 'class' => 'list-inline list-unstyled', 'title' => false, 'nextpage' => '<i class="fa fa-angle-right"></i>', 'previouspage' => '<i class="fa fa-angle-left"></i>' ) );    
    }
}
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
        ...
        $field_container = '<div class="form-group row %1$s" id="%2$s" data-priority="' . esc_attr( $sort ) . '"><div class="col-md-12">%3$s</div></div>';
		...
		switch ( $args['type'] ) {
			case 'country':
				...
				if ( 1 === count( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="form-control unicase-form-control country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . '><option value="">' . esc_html__( 'Select a country&hellip;', 'flipmart' ) . '</option>';
			...
			case 'state':
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field_container = '<div class="form-row form-group %1$s" id="%2$s" style="display: none"><div class="col-md-12">%3$s</div></div>';
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="form-control unicase-form-control state_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ? $args['placeholder'] : esc_html__( 'Select an option&hellip;', 'flipmart' ) ) . '">
					...
					$field .= '<input type="text" class="input-text form-control unicase-form-control ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" value="' . esc_attr( $value ) . '"  placeholder="' . esc_attr( $args['placeholder'] ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" ' . implode( ' ', $custom_attributes ) . ' />';

				}

				break;
			case 'textarea':
				$field .= '<textarea name="' . esc_attr( $key ) . '" class="input-text form-control unicase-form-control ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '" ' . ( empty( $args['custom_attributes']['rows'] ) ? ' rows="2"' : '' ) . ( empty( $args['custom_attributes']['cols'] ) ? ' cols="5"' : '' ) . implode( ' ', $custom_attributes ) . '>' . esc_textarea( $value ) . '</textarea>';
			...
			case 'url':
			case 'tel':
				$field .= '<input type="' . esc_attr( $args['type'] ) . '" class="input-text form-control unicase-form-control ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '"  value="' . esc_attr( $value ) . '" ' . implode( ' ', $custom_attributes ) . ' />';
			...
			case 'select':
				...
				if ( ! empty( $args['options'] ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="form-control unicase-form-control select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '">
							' . $options . '
						</select>';
				}
