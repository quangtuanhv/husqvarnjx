...
if ( ! class_exists( 'ReduxFramework_gradient' ) ) {
    class ReduxFramework_gradient {
        ...
        function render() {
			...
			printf( '<input type="text" class="hidden flipmart-gradient-css">', $this->field['id'] );
			printf( '<input type="text" class="hidden flipmart-gradient-bg">', $this->field['id'] );
			printf( '<div id="%1$s-gradient" class="flipmart-gradient"></div>', $this->field['id'] );
        }
        ...
        function enqueue() {
            ...
            if (!wp_script_is ( 'redux-field-gradient-js' )) {
				...
				$url = trailingslashit( get_template_directory_uri() ) . 'yog/extensions/redux-gradient';
