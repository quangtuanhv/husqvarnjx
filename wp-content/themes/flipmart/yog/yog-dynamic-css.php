...
/**
* Theme Framework
* The Yog_Dynamic_CSS initiate dynamic css.
*/

if( !defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

class Yog_Dynamic_CSS extends Yog_Base {
	...
	public function set_mode() {
		...
		$mode = yog_helper()->get_theme_option( 'dynamic_css_compiler' ) ? 'file' : 'inline';
		...
		if ( 'file' == $mode && $this->needs_update() ) {
			...
			$last_time    = (int) get_option( 'yog_dynamic_css_time' );
	...
	public function make_css() {
		...
		if ( ! $wp_filesystem->put_contents( $this->file( 'path' ), $content, FS_CHMOD_FILE ) ) {
			...
			$option  = get_option( 'yog_dynamic_css_posts', array() );
			$option[$page_id] = true;
			update_option( 'yog_dynamic_css_posts', $option );
	...
	public function needs_update() {
		...
		// Get the 'yog_dynamic_css_posts' option from the DB
		$option  = get_option( 'yog_dynamic_css_posts', array() );
		...
		// If the current page ID exists in the array of pages defined in the 'yog_dynamic_css_posts' option
	...
	public function can_write() {
		...
		$file_name   = '/yog-css' . $blog_id . '-' . $page_id . '.css';
		$folder_path = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'yog-styles';
	...
	public function file( $target = 'path' ) {
		...
		$css_uri = trailingslashit( $css_uri_folder ) . 'yog-styles/' . $file_name;
	...
	public function dynamic_css_cached() {
		 ...
		 */
		$cache = ( $cache && ( null == yog_helper()->get_theme_option( 'dynamic_css_db_caching' ) || ! yog_helper()->get_theme_option( 'dynamic_css_db_caching' ) ) ) ? false : $cache;
		...
		if( $cache ) {
			...
			/**
		 	 ...
		 	 */
			$transient_name = $page_id ? 'yog_dynamic_css_' . $page_id : 'yog_dynamic_css_global';
	...
	public function dynamic_css() {
		 ...
		 */
        if( yog_helper()->get_option( 'dynamic_css', 'raw', false, 'options' ) ){
            $dynamic_css = $this->dynamic_css_parser( yog_dynamic_css_array() );    
        }
		
		/**
		 * Append the user-entered dynamic CSS
		 */
		$dynamic_css .= wp_strip_all_tags( yog_helper()->get_theme_option( 'custom_css' ) );

		return $dynamic_css;
	}

	/**
	 * Get the array of dynamically-generated CSS and flipmart it to a string.
	...
	public function dynamic_css_parser( $css ) {
		...
		return apply_filters( 'yog_dynamic_css', $final_css );
	}
	...
	public function enqueue_dynamic_css() {
		...
		if ( 'file' == self::$mode ) {
			...
			wp_enqueue_style( 'yog-dynamic-css', $this->file( 'uri' ) );
		}
	}
	...
	public function add_inline_css() {
		...
		if ( ('inline' == self::$mode || $wp_customize) && $dynamic_css = $this->dynamic_css_cached() ) {
		    yog_theme_assets()->yog_inline_css_script( $dynamic_css );
		}
	...
	public function reset_post_transient( $post_id ) {
		delete_transient( 'yog_dynamic_css_' . $post_id );
	}

	/**
	 * Update the yog_dynamic_css_posts option when a post is saved.
	 */
	...
	public function add_options() {
		/**
		 * The 'yog_dynamic_css_posts' option will hold an array of posts that have had their css generated.
		 * We can use that to keep track of which pages need their CSS to be recreated and which don't.
		 */
		add_option( 'yog_dynamic_css_time', array(), '', 'yes' );
		/**
		 * The 'yog_dynamic_css_time' option holds the time the file writer was last used.
		 */
		add_option( 'yog_dynamic_css_time', time(), '', 'yes' );
	}
	...
	public function reset_all_transients() {
		...
		$sql = "DELETE FROM wp_options WHERE option_name LIKE '_transient_yog_dynamic_css_%'";
		$wpdb->query( $sql );
	}
	...
	public function clear_cache() {
		...
		if ( yog_helper()->get_theme_option( 'dynamic_css_compiler' ) && yog_helper()->get_theme_option( 'cache_server_ip' ) ) {
	...
	public function clear_varnish_cache( $url ) {
		...
		if ( yog_helper()->get_theme_option( 'cache_server_ip' ) ) {
			$varniship = yog_helper()->get_theme_option( 'cache_server_ip' );
	...
	/**
	 * Update the yog_dynamic_css_posts option when the theme options are saved.
	 * This basically empties the array of page IDS from the 'yog_dynamic_css_posts' option
	 */
	public function global_reset_option() {
		update_option( 'yog_dynamic_css_posts', array() );
	}
	...
	public function update_saved_time() {
		update_option( 'yog_dynamic_css_time', time() );
	}
}
