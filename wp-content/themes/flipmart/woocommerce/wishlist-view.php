		...
		<div class="col-md-12 my-wishlist">
        	<div class="table-responsive">
                ...
                <form id="yith-wcwl-form" action="<?php echo $form_action ?>" method="post" class="woocommerce">
                    ...
                    if( ! empty( $page_title ) ) :
                    ?>
                        <div class="wishlist-titles <?php echo ( $is_custom_list ) ? 'wishlist-title-with-form' : ''?>">
                            <?php echo apply_filters( 'yith_wcwl_wishlist_title', '<h2 class="heading-title">' . $page_title . '</h2>' ); ?>
                            <?php if( $is_custom_list ): ?>
                                <a class="btn button show-title-form">
                                    <?php echo apply_filters( 'yith_wcwl_edit_title_icon', '<i class="fa fa-pencil"></i>' )?>
                                    <?php _e( 'Edit title', 'flipmart' ) ?>
                                </a>
                            <?php endif; ?>
                        </div>
                        <?php if( $is_custom_list ): ?>
                    ...
                    <!-- WISHLIST TABLE -->
                	<table class="shop_table cart table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo $wishlist_id ?>" data-token="<?php echo $wishlist_token ?>">
                        ...
                        if( count( $wishlist_items ) > 0 ) :
                                ...
                                if( $product && $product->exists() ) :
                                    ...
                                    <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
                                        ...
                                        <td class="col-md-7 text-left">
                                            ...
                                            <?php if( $show_price ) : ?>
                                                <div class="price">
                                                    <?php echo $product->get_price() ? $product->get_price_html() : apply_filters( 'yith_free_text', __( 'Free!', 'flipmart' ) ); ?>
                                                </div>
                                            <?php endif ?>
                                            
                                            <?php if( $show_stock_status ) : ?>
                                                <div class="price">
                                                    <?php echo $stock_status == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'flipmart' ) . '</span>' : '<span class="wishlist-in-stock">' . __( 'In Stock', 'flipmart' ) . '</span>'; ?>
                                                </div>
                                            <?php endif ?>
                                            
                                            <?php
                	                        if( $show_dateadded && isset( $item['dateadded'] ) ):
                								echo '<span class="dateadded">' . sprintf( __( 'Added on : %s', 'flipmart' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
                	                        endif;
                	                        ?>
                	                        ...
                	                        <select class="change-wishlist selectBox">
                		                        <option value=""><?php _e( 'Move', 'flipmart' ) ?></option>
                		                        <?php
                		                        ...
                		                        ?>
                			                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
                				                        ...
                				                        if( $wl['wishlist_privacy'] == 1 ){
                					                        $wl_privacy = __( 'Shared', 'flipmart' );
                				                        }
                				                        elseif( $wl['wishlist_privacy'] == 2 ){
                					                        $wl_privacy = __( 'Private', 'flipmart' );
                				                        }
                				                        else{
                					                        $wl_privacy = __( 'Public', 'flipmart' );
                				                        }
                                        ...
                                        <td class="col-md-1 close-btn">
                                            <?php if( $is_user_owner ): ?>
                                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php _e( 'Remove this product', 'flipmart' ) ?>"><i class="fa fa-times"></i></a>
                                            <?php endif; ?>
                                            <?php if( $show_cb ) : ?>
                                            ...
                                            <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php _e( 'Remove this product', 'flipmart' ) ?>"><?php _e( 'Remove', 'flipmart' ) ?></a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php
                            ...
                            <tr>
                                <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products were added to the wishlist', 'flipmart' ) ) ?></td>
                            </tr>
                        <?php
                        endif;
                	        ...
                	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
                	            <?php if( $show_cb ) : ?>
                		            <div class="custom-add-to-cart-button-cotaniner">
                		                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_token ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', __( 'Add the selected products to the cart', 'flipmart' ) ) ?></a>
                		            </div>
                	            <?php endif; ?>
                	            ...
                	            <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
                		            <div class="ask-an-estimate-button-container">
                	                    ...
                	                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate', 'flipmart' ) ) ?>
                	                </a>
                		            </div>
                	            <?php endif; ?>
                	...
                	<div id="ask_an_estimate_popup">
                		<form action="<?php echo $ask_estimate_url ?>" method="post" class="wishlist-ask-an-estimate-popup">
                			<?php if( ! is_user_logged_in() ): ?>
                				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', __( 'Your email', 'flipmart' ) ) ?></label>
