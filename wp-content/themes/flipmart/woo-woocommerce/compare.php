...
<head>
    ...
    <style type="text/css">
        ...
        table.compare-list .add-to-cart td .btn-primary{
...
<!-- START BODY -->
<body <?php body_class('flipmart') ?>>

<h1>
    <?php echo $localized_table_text ?>
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php _e( 'Close window [X]', 'flipmart' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php _e( 'No products added in the compare table.', 'flipmart' ) ?></td>
        </tr>
        ...
        <tr class="remove">
            ...
            foreach( $products as $product_id => $product ) :
                ...
                <td class="<?php echo $product_class; ?>">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product_id ) ) ?>" data-product_id="<?php echo $product_id; ?>"><?php _e( 'Remove', 'flipmart' ) ?> <span class="remove">x</span></a>
                </td>
                <?php
                ++$index;
            endforeach;
                        ...
                        switch( $field ) {
                            ...
                            case 'add-to-cart':
                                ...
                                echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                                	sprintf( '<a rel="nofollow" href="%s" data-quantity="%s" data-product_id="%s" data-product_sku="%s" class="btn btn-primary icon %s">%s</a>',
                                		esc_url( $product->add_to_cart_url() ),
                                		esc_attr( isset( $quantity ) ? $quantity : 1 ),
                                		esc_attr( $product->get_id() ),
                                		esc_attr( $product->get_sku() ),
                                		esc_attr( isset( $class ) ? $class : 'button' ),
                                        esc_html__( 'Add Cart', 'flipmart' )
                                	),
                                $product ); 
                                break;
